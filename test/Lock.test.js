...
const provider = ethers.provider; // Provider allows us to communication with blockchain.
...
describe("Lock Contract", function () {
  ...
  it("Reverts exceeding transfer", async function () {
    await expect(
      token
        .connect(user1)
        .transfer(user2.address, ethers.utils.parseUnits("300", 18))
    ).to.be.reverted;
  });
  ...
  describe("Contract Functions", function () {
    ...
    it("User1 locks 10 tokens", async function () {
      ...
      expect(balances[3] + 10).to.be.equal(
        ethToNum(await token.balanceOf(lock.address))
      );
      expect(userLocks[0]).to.be.equal(
        ethToNum(await lock.lockers(user1.address))
      );
    });
